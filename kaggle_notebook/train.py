...
# ---- LOAD CORPUS ----
texts = []
for fname in sorted(os.listdir('/kaggle/input/datasets/diegovelandiabarajas-lang/rubidium-corpus-train/resources')):
    if fname.endswith('.txt'):
        with open(os.path.join('/kaggle/input/datasets/diegovelandiabarajas-lang/rubidium-corpus-train/resources', fname), 'r', encoding='utf-8', errors='replace') as f:
            texts.append(f.read())
full_text = '\n'.join(texts)
...
m.save('/kaggle/working/model_10m_final.pkl')
print('Saved model_10m_final.pkl')

# ---- QUICK TEST ----
for seed in ['Hola', 'Buenos dias', 'Quien eres', 'Que puedes hacer', 'Como estas', 'Don Quijote']:
    r = m.generate(seed, max_chars=120, temperature=0.7, top_k=40)
    print(f'{seed} -> {r}')
